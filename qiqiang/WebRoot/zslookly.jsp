...
<body>
<c:if test="${m_type=='1'}">
<p align="center"><font size="5" color="red">查看我的留言</font></p>
</c:if>
<c:if test="${m_type=='2'}">
<p align="center"><font size="5" color="red">查看意向组留言</font></p>
</c:if>
<c:if test="${m_type=='0'}">
<p align="center"><font size="5" color="red">查看未回访留言</font></p>
</c:if>
<c:if test="${empty messages}">
   无记录!
   </c:if>
   <c:if test="${!empty messages}">
 <form id="form1" name="form1" method="post" action="userlyaction.do?method=showMessages&pageNow=${pageNow}">
	   <p align="center">
	   <c:if test="${pageNow>1}">
	   <a href="userlyaction.do?method=showMessages&pageNow=1">第一页</a> || 
	   <a href="userlyaction.do?method=showMessages&pageNow=${pageNow-1}">上一页</a> ||
	   </c:if>
	   <c:if test="${pageNow<pageCount}">
	    <a href="userlyaction.do?method=showMessages&pageNow=${pageNow+1}">下一页</a> 
	    || <a href="userlyaction.do?method=showMessages&pageNow=${pageCount}">最后一页</a> 
	   </c:if>
...
<c:forEach items="${messages}" var="message">
    <table width="961" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#FF9900">
      <tr>
        ...
        <td width="165" rowspan="3" bgcolor="#FFFFFF">
        <c:if test="${userid==message.fp_user}">
        <iframe src="modifyren2.do?num=${message.fp_user}&id=${message.id}&pageNow = ${pageNow}&flag=${flag}&type=${message.type}" width="165" height="80" frameborder="0" scrolling="no"></iframe>
        </c:if>
        </td>
<!--         <td width="35" rowspan="3" bgcolor="#FFFFFF"><a href="usercomplete.do?method=doComplete&id=${message.id}&pageNow=${pageNow}">完成</a></td> -->
      </tr>
      <tr>
  ...
  <c:if test="${userid==message.fp_user}">
  <tr>
    <td><div align="right"><a href="userlyaction.do?method=modifyMessageView&id=${message.id}&pageNow=${pageNow}">修改回访</a></div></td>
  </tr>
  </c:if>
</table>
</td>
