...
<p align="center">查看已做客户 </p>
<c:if test="${empty messages}">
   无记录!
   </c:if>
   <c:if test="${!empty messages}">
   
	 <form id="form1" name="form1" method="post" action="complete.do?pageNow=${pageNow}&flag=0">
	   <p align="center">
	   <c:if test="${pageNow>1}">
	   <a href="complete.do?pageNow=1&flag=0">第一页</a> || 
	   <a href="complete.do?pageNow=${pageNow-1}&flag=0">上一页</a> ||
	   </c:if>
	   <c:if test="${pageNow<pageCount}">
	    <a href="complete.do?pageNow=${pageNow+1}&flag=0">下一页</a> 
	    || <a href="complete.do?pageNow=${pageCount}&flag=0">最后一页</a> 
	   </c:if>
