...
public class MessageService {
	...
	public int getCountByTime(String id,String time,String flag)
	{
		...
		if("history".equals(flag))
			sql = "select count(*) from khmessage where fp_user="+id+" and lasthf <>'' and lasthf<'"+time+"' and type =1";
		else
			sql = "select count(*) from khmessage where fp_user="+id+" and lasthf like '"+time+"%'";
		SqlHelper sqlhelper=new SqlHelper();
		ArrayList al=sqlhelper.excuteQuery(sql, null);
		Object[] obj=(Object[]) al.get(0);
		pageCount=Integer.parseInt(obj[0].toString());
	...
	public ArrayList<MessageBean> getMessagesByTime(String time,String id,int pageNow,int pageSize,String flag)
	{
		...
		if(!"history".equals(flag))
		{
				sql = "select * from khmessage  where fp_user="+id+" and lasthf like '"+time+"%' order by lasthf desc limit "+(pageNow-1)*pageSize+","+pageSize;
		}
		else	
			   sql = "select * from khmessage  where fp_user="+id+" and lasthf <> '' and lasthf<'"+time+"' and type=1 order by lasthf desc limit "+(pageNow-1)*pageSize+","+pageSize;
